...
def atualizar_heroi(dt):
    ...
    if keyboard.left:
        ...
        # Usar sprites de descanso com base na direção
        if heroi.direcao == -1:
    ...
    if keyboard.up and heroi.no_chao:
        ...
        if heroi.direcao == -1:
            heroi.image = sprites_heroi_pulo_esquerda[0]  # Usar sprite de pulo para esquerda
        else:
            heroi.image = sprites_heroi_pulo_direita[0]  # Usar sprite de pulo para direita
...
def atualizar_inimigos(dt):
    ...
    for inimigo in inimigos[:]:  # Usar uma cópia da lista para evitar problemas ao remover itens
        if inimigo.estado == "ferido":
            # Atualizar o tempo do zumbi ferido
            inimigo.tempo_ferido += dt
            if inimigo.tempo_ferido >= 1.0:  # Esperar 1 segundo antes de desaparecer
                inimigos.remove(inimigo)  # Remover o zumbi da lista
        else:
